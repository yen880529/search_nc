...
def search_excel(search_term1, search_type1, search_term2=None, search_type2=None):
    ...
    try:
        ...
        latest_file = get_latest_excel_file(folder_path)  # 假設這個函數正確返回最新檔案路徑
        ...
        # 指定需要讀取的工作表
        sheet_names = ["T51", "T22", "L40", "L43", "L50", "L51", "L52", "B11", "B12B32", "M31", "M41"]
        df_dict = pd.read_excel(latest_file, sheet_name=sheet_names, header=1)
        ...
        if not search_column_name1:
            messagebox.showerror("錯誤", f"找不到第一條件對應的欄位：{search_type1}")
            return []

        if search_term2 and search_type2:
            search_column_name2 = search_columns.get(search_type2.lower())
            if not search_column_name2:
                messagebox.showerror("錯誤", f"找不到第二條件對應的欄位：{search_type2}")
                return []
        ...
        for sheet_name, df in df_dict.items():
            ...
            # 確保欄位存在
            if search_column_name1 not in df.columns:
                continue
            ...
            # 排除不必要的欄位
            exclude_columns = ["完成日", "序號", "作業", "工單數量","預計完成日","實際開工日","實際完工日", "工單狀況", "機台開工", "客戶名稱", "距離開工日", "朴卦日程"]
        ...
        if results:
            ...
            messagebox.showinfo("提示", "沒有符合條件的資料。")
            return []
...
def open_choose_plan_search(root):
    search_window = tk.Toplevel(root)
    search_window.title("Excel 搜尋工具")
    search_window.configure(bg="#A6ECE0")  # 設置背景顏色
    ...
    # 搜尋條件下拉菜單
    tk.Label(search_window, text="搜尋類型(category)-1:", bg="#A6ECE0", font=default_font).grid(row=0, column=0, padx=10, pady=10, sticky="ew")
    ...
    # 搜尋輸入框
    tk.Label(search_window, text="搜尋名稱(search name)-1:", bg="#A6ECE0", font=default_font).grid(row=0, column=2, padx=10, pady=10, sticky="ew")
    search_entry1 = tk.Entry(search_window, font=default_font)
    search_entry1.grid(row=0, column=3, padx=10, pady=10, sticky="ew")


     # 第二行搜尋條件下拉菜單和輸入框
    tk.Label(search_window, text="搜尋類型(category)-2:", bg="#A6ECE0", font=default_font).grid(row=1, column=0, padx=10, pady=10, sticky="ew")
    ...
    tk.Label(search_window, text="搜尋名稱(search name)-2:", bg="#A6ECE0", font=default_font).grid(row=1, column=2, padx=10, pady=10, sticky="ew")
    ...
    style.configure("Treeview", rowheight=25)  # 設定每行高度

    # 定義表格列
    columns = ("批號", "工單號碼", "料號", "品名", "製程", "工作中心", "工時", "預計開始日", "圖號", "版次")
    ...
    # 設置窗口的網格佈局
    #search_window.grid_rowconfigure(2, weight=1)  # 讓第2行自適應高度
    #search_window.grid_columnconfigure(0, weight=1)  # 讓第0列自適應寬度

    # 設置右鍵菜單
    set_right_click_menu(result_tree)

    tk.Label(search_window, text=f"最新檔案日期：{latest_date}", bg="#A6ECE0", font=default_font, fg="#333").grid(row=3, column=0, columnspan=6, padx=10, pady=10, sticky="ew")
    
    # 未完成標籤
    canvas = tk.Canvas(search_window, width=20, height=20, bg="#A6ECE0", highlightthickness=0)
